...
def episode_servers(x):
    ...
    if isinstance(arr,list):
        for srv in arr:
            ...
            name=clean(srv.get('server_name') or srv.get('name') or 'Nguồn');data=srv.get('server_data') or []
            if isinstance(data,list):out.append((name,data))
    return h,out
def episode_rows(x):
...
def sort_newest(items):
    # Source page order wins. For items not ranked by the source, use reliable release year, then title.
    return sorted(items,key=lambda x:(-(year(x) or 0),clean(x.get('title','')).lower()))
def ordered_items(path,predicate=None):
    data,by=data_index();seen=set();out=[];ranked=source_order(path)
    for u in ranked:
        x=by.get(u)
...
def genre_options(data):
    ...
    out=[]
    for p in preferred:
        if p in found and p not in out:out.append(p)
    out.extend(sorted(g for g in found if g not in out))
    return out
...
def manifest_data():
    ...
    return {'id':'community.ivy.catalog','version':'1.1.0','name':'Ivy❤️','description':'Ivy❤️ • Movies, series, seasons and episodes','resources':['catalog','meta','stream'],'types':['movie','series'],'idPrefixes':['ivy_'],'behaviorHints':{'configurable':False},'catalogs':[
...
@app.get('/')
def root():return jsonify({'ok':True,'service':'Ivy❤️','version':'1.1.0','manifest':'/manifest.json'})
@app.get('/manifest.json')
def manifest():return jsonify(manifest_data())
@app.get('/health')
def health():
    d=load();return jsonify({'ok':True,'version':'1.1.0','movies':len(d.get('movies',[])),'latest':len(source_order('/phimhay')),'movieOrder':len(source_order('/phim-le')),'seriesOrder':len(source_order('/phim-bo'))})
