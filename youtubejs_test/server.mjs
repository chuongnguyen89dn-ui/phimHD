...
async function discoverCdn(){for(const ep of DISCOVERY){try{const r=await fetch(ep,{headers:{'User-Agent':UA,Accept:'application/json',Origin:'https://yt.savetube.me',Referer:'https://yt.savetube.me/'},signal:signal(4000)});const j=await r.json();if(r.ok&&j?.cdn)return j.cdn;}catch{}}throw new Error('No SaveTube CDN');}
async function getCtx(id,cdn){const ck=`${id}@${cdn}`;const hit=ctxCache.get(ck);if(hit&&hit.expires>Date.now())return hit.ctx;const base=`https://${cdn}`;const headers={'Content-Type':'application/json',Accept:'application/json','User-Agent':UA,Origin:'https://yt.savetube.me',Referer:'https://yt.savetube.me/'};const r=await fetch(`${base}/v2/info`,{method:'POST',headers,body:JSON.stringify({url:`https://www.youtube.com/watch?v=${id}`}),signal:signal(8000)});const j=await r.json();if(!r.ok||!j?.data)throw new Error(`info ${r.status}`);const info=decrypt(j.data);const ctx={base,headers,info,cdn};ctxCache.set(ck,{ctx,expires:Date.now()+20*60*1000});return ctx;}
async function doResolve(id,q){const cdn=await discoverCdn();const c=await getCtx(id,cdn);const r=await fetch(`${c.base}/download`,{method:'POST',headers:c.headers,body:JSON.stringify({id,downloadType:'video',quality:q,key:c.info.key}),signal:signal(10000)});const t=await r.text();let j=null;try{j=JSON.parse(t)}catch{}const url=j?.data?.downloadUrl||j?.data?.url||j?.downloadUrl;if(!r.ok||!url)throw new Error(`download ${r.status}`);markVerified(id,q);mediaCache.set(`${id}:${q}`,{url,provider:`savetube:${cdn}`,expires:Date.now()+2*60*60*1000});console.log('[PLAY-RESOLVED]',id,q,cdn);return url;}
...
const manifest={id:ADDON_ID,version:'8.6.0',name:'Ivy ❤️ Khoai Lang Thang',description:'Safe playback mode while dedicated YouTube mux backend is being tested. No background quality probes.',resources:['catalog','meta','stream'],types:['movie'],catalogs:[{type:'movie',id:CATALOG_ID,name:'Khoai Lang Thang • Full Channel'}],idPrefixes:['yt:']};
...
server.listen(PORT,'0.0.0.0',()=>{console.log('Ivy Khoai Lang Thang safe fallback listening',PORT);void refreshCatalog(true);});
