...
if(path==='/')return send(res,200,{service:manifest.name,version:manifest.version,videos:videoMap.size,loading:catalogLoading});
...
if(path==='/diag.json')return send(res,200,{videos:videoMap.size,catalogLoading,catalogLoadedAt,mediaCache:[...mediaCache.keys()]});
...
const sm=path.match(/^\/stream\/movie\/yt:([A-Za-z0-9_-]{11})\.json$/);if(sm){const id=sm[1];const streams=QUALITIES.map(q=>({name:`Ivy YouTube • ${q==='2160'?'4K ':''}${q}p`,title:`Khoai Lang Thang • ${q}p`,url:`${ORIGIN}/play/${id}/${q}.mp4`}));console.log('[STREAM-INSTANT]',id,streams.length);return send(res,200,{streams});}
...
server.listen(PORT,'0.0.0.0',()=>{console.log('Ivy Khoai Lang Thang instant-stream addon listening',PORT);void refreshCatalog(true);});
