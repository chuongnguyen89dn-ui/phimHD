...
const PIPED=['https://pipedapi.kavin.rocks','https://pipedapi.leptons.xyz','https://pipedapi.adminforge.de','https://api.piped.private.coffee'];
...
async function getPipedInfo(id){const hit=pipedCache.get(id);if(hit&&hit.expires>Date.now())return hit.data;for(const base of PIPED){try{const r=await fetch(`${base}/streams/${id}`,{headers:{Accept:'application/json','User-Agent':UA},signal:signal(7000)});if(!r.ok)continue;const j=await r.json();if(Array.isArray(j?.videoStreams)){pipedCache.set(id,{data:{...j,_base:base},expires:Date.now()+10*60*1000});console.log('[PIPED-INFO]',id,base,j.videoStreams.length);return {...j,_base:base};}}catch(e){console.log('[PIPED-INSTANCE-MISS]',id,base,String(e));}}throw new Error('No Piped instance');}
function pipedQuality(s){const m=String(s?.quality||'').match(/(\d{3,4})p?/);return m?m[1]:String(s?.height||'');}
async function resolvePiped(id,q){const j=await getPipedInfo(id);const matches=(j.videoStreams||[]).filter(s=>s?.url&&pipedQuality(s)===q&&!s.videoOnly).sort((a,b)=>(b.bitrate||0)-(a.bitrate||0));if(matches[0]?.url)return matches[0].url;throw new Error(`Piped no muxed ${q}p`);}
async function getPipedDash(id){const j=await getPipedInfo(id);return j?.dash||null;}

async function getCdn(){for(const ep of DISCOVERY){try{const r=await fetch(ep,{headers:{'User-Agent':UA,Accept:'application/json',Origin:'https://yt.savetube.me',Referer:'https://yt.savetube.me/'},signal:signal(6000)});const j=await r.json();if(r.ok&&j?.cdn)return j.cdn;}catch{}}throw new Error('No SaveTube CDN');}
async function getCtx(id){const cached=ctxCache.get(id);if(cached&&cached.expires>Date.now())return cached.ctx;const cdn=await getCdn();const base=`https://${cdn}`;const headers={'Content-Type':'application/json',Accept:'application/json','User-Agent':UA,Origin:'https://yt.savetube.me',Referer:'https://yt.savetube.me/'};const r=await fetch(`${base}/v2/info`,{method:'POST',headers,body:JSON.stringify({url:`https://www.youtube.com/watch?v=${id}`}),signal:signal(12000)});const j=await r.json();if(!r.ok||!j?.data)throw new Error(`info ${r.status}`);const info=decrypt(j.data);if(info?.title){const old=videoMap.get(id)||{id};videoMap.set(id,{...old,id,title:info.title});}const ctx={base,headers,info};ctxCache.set(id,{ctx,expires:Date.now()+15*60*1000});return ctx;}
async function resolveSaveTubeStrict(id,q){const c=await getCtx(id);const r=await fetch(`${c.base}/download`,{method:'POST',headers:c.headers,body:JSON.stringify({id,downloadType:'video',quality:q,key:c.info.key}),signal:signal(18000)});const t=await r.text();let j=null;try{j=JSON.parse(t)}catch{}const url=j?.data?.downloadUrl||j?.data?.url||j?.downloadUrl;if(r.ok&&url)return url;throw new Error(`savetube ${r.status}`);}

async function resolveMedia(id,q){const key=`${id}:${q}`;const hit=mediaCache.get(key);if(hit&&hit.expires>Date.now()){console.log('[CACHE-HIT]',key,hit.provider);return hit.url;}try{const url=await resolvePiped(id,q);mediaCache.set(key,{url,provider:'piped',expires:Date.now()+45*60*1000});console.log('[PLAY-RESOLVED]',id,q,'piped');return url;}catch(e){console.log('[PIPED-MISS]',id,q,String(e));}try{const url=await resolveSaveTubeStrict(id,q);mediaCache.set(key,{url,provider:'savetube',expires:Date.now()+45*60*1000});console.log('[PLAY-RESOLVED]',id,q,'savetube');return url;}catch(e){console.log('[SAVETUBE-MISS]',id,q,String(e));throw new Error(`No exact ${q}p media URL`);}}

const manifest={id:ADDON_ID,version:'8.4.0',name:'Ivy ❤️ Khoai Lang Thang',description:'Piped-first playback with SaveTube fallback; no aggressive prefetch.',resources:['catalog','meta','stream'],types:['movie'],catalogs:[{type:'movie',id:CATALOG_ID,name:'Khoai Lang Thang • Full Channel'}],idPrefixes:['yt:']};
...
if(path==='/')return send(res,200,{service:manifest.name,version:manifest.version,videos:videoMap.size,loading:catalogLoading,mediaCache:mediaCache.size,pipedCache:pipedCache.size});
...
if(path==='/diag.json')return send(res,200,{videos:videoMap.size,catalogLoading,catalogLoadedAt,mediaCache:[...mediaCache.entries()].map(([k,v])=>({key:k,provider:v.provider})),pipedCache:[...pipedCache.keys()],ctxCache:[...ctxCache.keys()]});
...
const sm=path.match(/^\/stream\/movie\/yt:([A-Za-z0-9_-]{11})\.json$/);if(sm){const id=sm[1];const streams=[{name:'Ivy YouTube • AUTO/DASH',title:'Khoai Lang Thang • adaptive (Piped)',url:`${ORIGIN}/dash/${id}.mpd`},...QUALITIES.map(q=>({name:`Ivy YouTube • ${q==='2160'?'4K ':q==='1440'?'2K ':''}${q}p`,title:`Khoai Lang Thang • exact ${q}p`,url:`${ORIGIN}/play/${id}/${q}.mp4`}))];console.log('[STREAM-INSTANT]',id,streams.length);return send(res,200,{streams});}
const dm=path.match(/^\/dash\/([A-Za-z0-9_-]{11})\.mpd$/);if(dm){const dash=await getPipedDash(dm[1]);if(!dash)throw new Error('No Piped DASH');res.writeHead(302,{Location:dash,'Cache-Control':'no-store','Access-Control-Allow-Origin':'*'});return res.end();}
...
server.listen(PORT,'0.0.0.0',()=>{console.log('Ivy Khoai Lang Thang Piped-first addon listening',PORT);void refreshCatalog(true);});
