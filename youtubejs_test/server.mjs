...
const manifest={id:ADDON_ID,version:'11.1.3',name:'Ivy ❤️ Khoai Lang Thang',description:'Independent source quality with playable low-resolution fallback.',resources:['catalog','meta','stream'],types:['movie'],catalogs:[{type:'movie',id:CATALOG_ID,name:'Khoai Lang Thang • Full Channel'}],idPrefixes:['yt:']};
http.createServer(async(req,res)=>{
 try{
  ...
  if(m){
   const id=m[1];
   const [src,tr]=await Promise.all([source(id),transport(id)]);
   const arr=[];
   const tq=new Map((tr?.qualities||[]).map(x=>[x.quality,x]));
   if(src.ok){
    const genuine=src.qualities.filter(q=>tq.has(q));
    if(genuine.length){
     const top=genuine[0],x=tq.get(top);
     arr.push({name:`AUTO • ${top}p`,title:`Source verified ${top}p • ${x.codec}`,url:`${MUX}/stream/${id}/${top}.mp4`,behaviorHints:{notWebReady:true}});
     for(const q of genuine){const z=tq.get(q);arr.push({name:`Ivy YouTube • ${q}p`,title:`Source verified ${q}p • ${z.codec}`,url:`${MUX}/stream/${id}/${q}.mp4`,behaviorHints:{notWebReady:true}})}
     console.log('[AUTO-SOURCE]',id,src.max);
    }
   }
   if(!arr.length){
    const safe=[360,240,144].filter(q=>tq.has(q));
    for(const q of safe){const z=tq.get(q);arr.push({name:`Ivy YouTube • ${q}p`,title:`PLAYABLE FALLBACK • source max not verified • ${z.codec}`,url:`${MUX}/stream/${id}/${q}.mp4`,behaviorHints:{notWebReady:true}})}
    console.log('[SOURCE-BLOCKED-FALLBACK]',id,safe.join(','));
   }
...
}).listen(PORT,'0.0.0.0',()=>{console.log('[START v11.1.3]',PORT);void refresh()});
