...
const manifest={id:ADDON_ID,version:'11.1.1',name:'Ivy ❤️ Khoai Lang Thang',description:'Source quality is separated from provider transport quality. AUTO is never inferred from provider 4K alone.',resources:['catalog','meta','stream'],types:['movie'],catalogs:[{type:'movie',id:CATALOG_ID,name:'Khoai Lang Thang • Full Channel'}],idPrefixes:['yt:']};
http.createServer(async(req,res)=>{
 try{
  if(req.method==='OPTIONS'){res.writeHead(204,{'access-control-allow-origin':'*'});return res.end()}
  let p=decodeURIComponent(new URL(req.url,'http://x').pathname);
  if(p==='/')return send(res,200,{service:manifest.name,version:manifest.version,videos:videoMap.size});
  if(p==='/manifest.json')return send(res,200,manifest);
  if(p===`/catalog/movie/${CATALOG_ID}.json`){void refresh();return send(res,200,{metas:[...videoMap.values()].map(meta)})}
  let m=p.match(/^\/meta\/movie\/yt:([\w-]{11})\.json$/);
  if(m)return send(res,200,{meta:meta(videoMap.get(m[1])||{id:m[1]})});
  m=p.match(/^\/stream\/movie\/yt:([\w-]{11})\.json$/);
  if(m){
   let id=m[1],[src,tr]=await Promise.all([source(id),transport(id)]),arr=[],tq=new Map((tr?.qualities||[]).map(x=>[x.quality,x]));
   if(src.ok){
    let genuine=src.qualities.filter(q=>tq.has(q));
    if(genuine.length){
     let top=genuine[0],x=tq.get(top);
     arr.push({name:`AUTO • ${top}p`,title:`YouTube source max ${top}p • transport ${x.width}x${x.height} ${x.codec}`,url:`${MUX}/stream/${id}/${top}.mp4`,behaviorHints:{notWebReady:true}});
     for(const q of genuine){let z=tq.get(q);arr.push({name:`Ivy YouTube • ${q}p`,title:`YouTube source ${q}p • ${z.width}x${z.height} ${z.codec}`,url:`${MUX}/stream/${id}/${q}.mp4`,behaviorHints:{notWebReady:true}})}
    }
    console.log('[AUTO-SOURCE]',id,'sourceMax',src.max,'transport',[...tq.keys()].join(','));
   }else console.log('[AUTO-BLOCKED]',id,'No independent YouTube source-quality result; provider qualities not exposed as AUTO');
   return send(res,200,{streams:arr});
  }
  return send(res,404,{error:'not found'});
 }catch(e){console.log('[ERROR]',String(e));return send(res,502,{error:String(e)})}
}).listen(PORT,'0.0.0.0',()=>{console.log('[START v11.1.1]',PORT);void refresh()});
